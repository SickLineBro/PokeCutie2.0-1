...
module.exports = {
    ...
    execute: async (client, message, args, prefix, guild, color, channel) => {
        ...
        if (!user || !user.pokemons[0]) return message.channel.send("You need to pick a starter pok̩mon using the \`" + prefix.toLowerCase() + "start\` command before using this command!");
        ...
        if (!user2 || !user2.pokemons[0]) return message.channel.send(user1.user.username + " need to pick a starter pok̩mon using the \`" + prefix.toLowerCase() + "start\` command before using this command!");
;
