...
let user = await User.findOne({ id: message.author.id });
 ...
 let name = names[Math.floor(Math.random() * names.length)]
    ...
    if (gen <= 10) shiny = true;
       ...
       await spawn.save()
      user.oldhroduse = user.oldroduse + 1
      await user.save()
