...
module.exports = {
 ...
 let txt,
    ...
    if (isNaN(e[0])) txt = s.map((item, i) => `\`${item.num}\` - ${item.shiny ? "✨ " + (emojis[item.name.toLowerCase()+"_sh"] ? emojis[item.name.toLowerCase()+"_sh"] : "") : ""}**${(!item.shiny && emojis[item.name.toLowerCase()]) ? emojis[item.name.toLowerCase()] : ""} ${item.name.replace(/-+/g, " ").replace(/\b\w/g, l => l.toUpperCase())}** - Level: ${item.level} - IV: ${item.totalIV}%${(item.nick != null ? ` - Nickname: ${item.nick}` : "")}`).slice(0, 20).join("\n")
    if (Number(args[0])) {
      ...
      embed
        ...
        .setColor(color)
        .setDescription((chunks[index].map((item, i) => { return `\`${item.num}\` - ${item.shiny ? "✨ " + (emojis[item.name.toLowerCase()+"_sh"] ? emojis[item.name.toLowerCase()+"_sh"] : "") : ""}**${(!item.shiny && emojis[item.name.toLowerCase()]) ? emojis[item.name.toLowerCase()] : ""} ${item.name.replace(/-+/g, " ").replace(/\b\w/g, l => l.toUpperCase())}** - Level: ${item.level} - IV: ${item.totalIV}%${(item.nick != null ? ` - Nickname: ${item.nick}` : "")}` }).join("\n")))
      ...
      embed.setFooter(`Showing ${args[0]}-${chunks.length} out of ${s.length}.`);
      ...
      embed
        ...
        .setColor(color)
        .setDescription(txt)
       .setFooter(`Showing 1-${chunks.length} out of ${s.length}.`);
