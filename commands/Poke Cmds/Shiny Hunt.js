  ...
  execute: async (client, message, args, prefix, guild, color, channel) => {
    ...
    try {
      if (args[0].toLowerCase() == "reset") {
        ...
        await msg.react("1011494475928965150")
        msg.react("1011494453720137778")

        const collector = msg.createReactionCollector((reaction, userx) => ['1011494475928965150', '1011494453720137778'].includes(reaction.emoji.id) && userx.id === message.author.id, { time: 30000 })

        collector.on('collect', async (reaction, userx) => {
          if (reaction.emoji.id === "1011494475928965150") {
         ...
         Shinyhunt Resetted!`)
          } else if (reaction.emoji.id === "1011494453720137778") {
        ...
        if (user.shcount == 0 || user.shname == null) {
           ...
           **You are now Shiny Hunting \`${name.replace(/-+/g, " ").replace(/\b\w/g, l => l.toUpperCase())}\`!**`)
            } else if (reaction.emoji.id === "1011494453720137778") {
              collector.stop("aborted")
              msg.reactions.removeAll()
              message.channel.send("Cancelled.")
      ...
      let Name = args.join("-").toLowerCase()
      let name = Name.toLowerCase()
      if (error.toString().startsWith("StatusCodeError")) return message.channel.send(`> :x: **Could not find a pokémon in our Database matching \`${name}\`.**`)
