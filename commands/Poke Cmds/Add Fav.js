...
module.exports = {
    ...
    //  console.log(user.pokemons.filter(f=>f.fav === true))
      return message.channel.send(`>  <:check:872833782338715678> Added \`${user.pokemons[num].name} Number ${num + 1} \` Pokemon in your Favorites List!`);
    }
   ...
   if(args[0].toLowerCase()==='r'){
    ...
    let num = args[1] - 1;
      ...
      return message.channel.send(`> <:xmark3512:860202114419654657> \`${user.pokemons[num].name} Number ${num + 1}\` is not present in your Favorite list!`)
    }else{
      user.pokemons[num].fav = false;
      await User.findOneAndUpdate({id: message.author.id}, {pokemons: user.pokemons}, {new: true});
   //   console.log(user.pokemons.filter(f=>f.fav === true))
      return message.channel.send(`>  <:check:872833782338715678> Removed \`${user.pokemons[num].name} Number ${num + 1}\` Pokemon from your Favorites List!`);
    }
