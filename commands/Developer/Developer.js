  ...
  execute: async (client, message, args, prefix, guild, color, channel) => {
    ...
    const embed = new MessageEmbed()
      ...
      .addField("Balance",
        ...
        + `-> ${prefix}dev removebal(bal/credits/credit/cr) <@user/userId/userName> : Remove balance from an user.\n\`\`\``
      )
      .addField("Redeem",
        `\`\`\`\n-> ${prefix}dev giveredeem(redeems/r) <@user/userId/userName> [amount] : Give redeem to an user.\n`
        + `-> ${prefix}dev removeredeem(redeems/r) <@user/userId/userName> : Remove redeems from an user.\n\`\`\``
      )
      .addField("Shards",
        `\`\`\`\n-> ${prefix}dev giveshards(shard/sh) <@user/userId/userName> [amount] : Give shards to an user.\n`
        + `-> ${prefix}dev removeshards(shard/sh) <@user/userId/userName> : Remove shards from an user.\n\`\`\``
      )
      .addField("Votes",
      ...
      .addField("Pokémon",
        ...
        + `-> ${prefix}check-pk(cp) <@user/userId/userName> [--filters] : Check an user's pokémon.\n\`\`\``
      )

     .addField(`${prefix}wipe <mention/userid/name> [reason]`, `\`\`\`Clear user data from Database\`\`\``)
     .addField(`${prefix}check-pf <mention/userid/name>`)
    ...
    if (cmd == "suspend") { // SUSPEND
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          ...
          .setTitle(`Account Unsuspended`)
          .setDescription(`You have been unuspended from ${client.user.username}!`)
...
//REMOVE BAL
    ...
    } else if (cmd == "removebal") { //REMOVEBAL

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to remove  **${amount}** balance of \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          ...
          await userx.save()
          let givebal_embed = new MessageEmbed()
          .setDescription(`**${amount}** credits have been removed from your balance from ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          user.send(givebal_embed).catch(e => {
            if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
...
//REMOVEBRONZE
      } else if (cmd == "rembcrate") { // Diamondcrate

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to remove **${amount}** bronze crate from \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          collector.stop("given")
          msg.reactions.removeAll()
          userx.bronzecrate = userx.bronzecrate - amount
          await userx.save()
          let bronzecrate_embed = new MessageEmbed()
          .setDescription(`**${amount}** bronze crates has been removed from yout account by ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          user.send(bronzecrate_embed).catch(e => {  if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
          })
          return message.channel.send(`Success.`)
        } else if (reaction.emoji.name === "❌") {
...
//GIVEBRONZE
...
} else if (cmd == "givebcrate") { // Diamondcrate

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to give **${amount}** bronze crate to \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          ...
          await userx.save()
          let bronzecrate_embed = new MessageEmbed()
          .setDescription(`You have received **${amount}** bronze crates from ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          user.send(bronzecrate_embed).catch(e => {  if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
          })
...
//REMOVESILVER
} else if (cmd == "removescrate") { // Diamondcrate

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to remove **${amount}** silver crate from \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          collector.stop("given")
          msg.reactions.removeAll()
          userx.silvercrate = userx.silvercrate - amount
          await userx.save()
          let silvercrate_embed = new MessageEmbed()
          .setDescription(` **${amount}** silver crates has been removed from your account by ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          user.send(silvercrate_embed).catch(e => {  if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
          })
          return message.channel.send(`Success.`)
        } else if (reaction.emoji.name === "❌") {
...
//GIVESILVER
} else if (cmd == "givescrate") { // Diamondcrate

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to give **${amount}** silver crate to \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          ...
          await userx.save()
          let silvercrate_embed = new MessageEmbed()
          .setDescription(`You have received **${amount}** silver crates from ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          user.send(silvercrate_embed).catch(e => {  if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
          })
...
//REMOVEGOLDEN
} else if (cmd == "removegcrate") { // Diamondcrate

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to remove **${amount}** golden crate from \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          collector.stop("given")
          msg.reactions.removeAll()
          userx.goldencrate = userx.goldencrate - amount
          await userx.save()
          let goldencrate_embed = new MessageEmbed()
          .setDescription(` **${amount}** golden crates has been removed from your account by ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          user.send(goldencrate_embed).catch(e => {  if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
          })
          return message.channel.send(`Success.`)
        } else if (reaction.emoji.name === "❌") {
...
// GIVEGOLD
      } else if (cmd == "givegcrate") { // Diamondcrate

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to give **${amount}** golden crate to \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          ...
          await userx.save()
          let goldencrate_embed = new MessageEmbed()
          .setDescription(`You have received **${amount}** golden crates from ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          user.send(goldencrate_embed).catch(e => {  if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
          })
...
// REMOVEDIAM
      } else if (cmd == "remdcrate") { // Diamondcrate

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to remove **${amount}** diamond crate from \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          collector.stop("given")
          msg.reactions.removeAll()
          userx.diamondcrate = userx.diamondcrate - amount
          await userx.save()
          let diamondcrate_embed = new MessageEmbed()
          .setDescription(` **${amount}** diamond crates has been removed from ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          user.send(diamondcrate_embed).catch(e => {  if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
          })
          return message.channel.send(`Success.`)
        } else if (reaction.emoji.name === "❌") {
...
//GIVEDELUXE
      ...
      } else if (cmd == "deluxecrate") { // Diamondcrate

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to give **${amount}** deluxe crate to \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          collector.stop("given")
          msg.reactions.removeAll()
          userx.deluxecrate = userx.deluxecrate + amount
          await userx.save()
          let deluxecrate_embed = new MessageEmbed()
          .setDescription(`You have received **${amount}** deluxe crates from ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          user.send(diamondcrate_embed).catch(e => {  if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
          })
    ...
    } else if (cmd == "diamondcrate") { // Diamondcrate

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to give **${amount}** diamond crate to \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          ...
          await userx.save()
          let diamondcrate_embed = new MessageEmbed()
          .setDescription(`You have received **${amount}** diamond crates from ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          user.send(diamondcrate_embed).catch(e => {  if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
          })
...
//REMOVESHARDS
    } else if (cmd == "removeshards") { //removeshard

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to remove **${amount}** shards from \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          ...
          await userx.save()
          let giveshards_embed = new MessageEmbed()
          .setDescription(` **${amount}** Shards has been removed from the ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          .setColor(color)
          user.send(giveredeem_embed).catch(e => {   if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
      ...
      //REMOVEREDEEM
    } else if (cmd == "removeredeem") { //removeredeem

      let userx = await User.findOne({ id: user.id })
      if (!userx) return message.channel.send(`\`\`\`\nError: User not registered in Db.\n\`\`\``)
      let amount = parseInt(args[2])

      let msg = await message.channel.send(`Do you confirm to remove **${amount}** redeems from \`${u} ${user.id}\` - <@${user.id}> ?`)
      await msg.react("✅")
      msg.react("❌")
      ...
      collector.on('collect', async (reaction, userxx) => {
        if (reaction.emoji.name === "✅") {
          ...
          await userx.save()
          let giveredeem_embed = new MessageEmbed()
          .setDescription(` **${amount}** Redeems has been removed from the ${client.user.username} Team!`)
          .addField("Join the Support Server", `[Click Here](${config.server})`)
          .setColor(color)
          user.send(giveredeem_embed).catch(e => {   if (e.message.toLowerCase() === "cannot send messages to this user") return message.channel.send("I wasn't able to DM them.")
            return
